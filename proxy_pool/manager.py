...
class ProxyManager:
    ...
    def get_proxy(self, exclude_list: List[str] = None) -> Optional[ProxyInfo]:
        ...
        try:
            ...
            for proxy_url, proxy_data in all_proxies.items():
                proxy_url = proxy_url.decode() if isinstance(proxy_url, bytes) else proxy_url
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'httpbin.org/headers' in test_url:
                            ...
                            elif proxy_info.ip not in headers.get('X-Forwarded-For', ''):
    ...
    def update_proxy_stats(self, proxy_url: str, success: bool, response_time: float = 0):
        ...
        try:
            ...
            # 重新计算成功率
            proxy_info.success_rate = proxy_info.successful_requests / proxy_info.total_requests
    ...
    def _start_background_tasks(self):
        ...
        # 启动后台线程
        validation_thread = threading.Thread(target=validate_proxies_periodically, daemon=True)
            ...
            for proxy_url, proxy_data in all_proxies.items():
                try:
                    ...
                    if time.time() - proxy_info.last_check > 300:
                        future = self.executor.submit(self._validate_and_update, proxy_info)
    ...
    def _validate_and_update(self, proxy_info: ProxyInfo):
        ...
        try:
            if self.validate_proxy(proxy_info):
                # 更新代理信息
                updated_data = json.dumps(asdict(proxy_info))
                self.redis_client.hset(self.proxy_pool_key, proxy_info.url, updated_data)
            else:
                # 移除无效代理
