...
class EpidemicDataItem(scrapy.Item):
    ...
    confirmed_cases = scrapy.Field(
        input_processor=MapCompose(parse_number), output_processor=TakeFirst()
    )
    ...
    # 数据质量
    data_quality_score = scrapy.Field(output_processor=TakeFirst())
    validation_status = scrapy.Field(output_processor=TakeFirst())
...
class NewsItem(scrapy.Item):
    ...
    tags = scrapy.Field(input_processor=MapCompose(clean_text))
...
class PolicyItem(scrapy.Item):
    ...
    # 关键词
    keywords = scrapy.Field(input_processor=MapCompose(clean_text))
