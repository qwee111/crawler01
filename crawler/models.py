...
from sqlalchemy import (
    ...
    Text, Boolean, JSON, ARRAY, CheckConstraint, UniqueConstraint,
    Index, func
)
...
class EpidemicData(Base):
    ...
    # 约束
    __table_args__ = (
        CheckConstraint('data_quality_score >= 0 AND data_quality_score <= 1', name='chk_quality_score'),
...
class CrawlerTask(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'running', 'completed', 'failed', 'cancelled')", name='chk_task_status'),
