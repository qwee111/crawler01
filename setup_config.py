...
class ConfigWizard:
    ...
    def __init__(self):
        ...
        self.env_example_path = self.project_root / "deployment" / "docker" / ".env.example"
    ...
    def ask_input(self, question: str, default: str = "", required: bool = False, 
    ...
    def configure_environment(self):
        ...
        self.config['DEBUG'] = 'true' if self.config['ENVIRONMENT'] == 'development' else 'false'
        self.config['LOG_LEVEL'] = 'DEBUG' if self.config['ENVIRONMENT'] == 'development' else 'INFO'
    ...
    def configure_databases(self):
        ...
        if self.ask_yes_no("自动生成PostgreSQL密码?"):
            ...
            self.config['POSTGRES_PASSWORD'] = self.ask_input("PostgreSQL密码", required=True, password=True)
        ...
        if self.ask_yes_no("自动生成MongoDB密码?"):
            ...
            self.config['MONGODB_ROOT_PASSWORD'] = self.ask_input("MongoDB密码", required=True, password=True)
    ...
    def configure_security(self):
        ...
        if self.ask_yes_no("自动生成MongoDB管理界面密码?"):
            ...
            self.config['MONGO_EXPRESS_PASSWORD'] = self.ask_input("MongoDB Express密码", password=True)
        ...
        if self.ask_yes_no("自动生成MinIO密码?"):
            ...
            self.config['MINIO_ROOT_PASSWORD'] = self.ask_input("MinIO密码", password=True)
    ...
    def configure_notifications(self):
        ...
        if self.ask_yes_no("配置邮件告警?", False):
            ...
            self.config['SMTP_PASSWORD'] = self.ask_input("邮箱密码/应用密码", required=True, password=True)
            self.config['SMTP_FROM'] = self.ask_input("发件人邮箱", self.config['SMTP_USERNAME'])
        else:
            self.config.update({
        ...
        if self.ask_yes_no("配置Slack告警?", False):
            print("Slack配置:")
            self.config['SLACK_WEBHOOK_URL'] = self.ask_input("Slack Webhook URL", required=True)
